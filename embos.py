...
#
# EMBOSS matcher (REST) web service Python client using xmltramp2.
#
...
parser.add_option('--alternatives', type=int, help=('Show additional alignments'))
...
def getResult(jobId, is_write_to_file: bool = True):
    ...
    for resultType in resultTypes:
        ...
        for outformat_type in outformat_parm:
            ...
            if not outformat_type or outformat_type == unicode(resultType[u'identifier']):
                ...
                if is_write_to_file:
                    _write_to_file(filename=filename, fmode=fmode, text=result)
                else:
                    return result
...
def _write_to_file(filename, fmode, text):
    try:
...
def print_usage():
    print("""\
EMBL-EBI EMBOSS matcher Python Client:
...
[Optional]
  ...
  --gapext              Pairwise alignment score for each additional residue in a
                        gap.
  --alternatives        Show additional alignments.
...
[General]
  ...
  --baseUrl             Base URL. Defaults to:
                        https://www.ebi.ac.uk/Tools/services/rest/emboss_matcher

Synchronous job:
  The results/errors are returned as soon as the job is finished.
  Usage: python emboss_matcher.py --email <your@email.com> [options...] <SeqFile|SeqID(s)>
  Returns: results as an attachment

Asynchronous job:
  Use this if you want to retrieve the results at a later time. The results
  are stored for up to 24 hours.
  Usage: python emboss_matcher.py --asyncjob --email <your@email.com> [options...] <SeqFile|SeqID(s)>
  Returns: jobid

Check status of Asynchronous job:
  Usage: python emboss_matcher.py --status --jobid <jobId>

Retrieve job data:
  Use the jobid to query for the status of the job. If the job is finished,
  it also returns the results/errors.
  Usage: python emboss_matcher.py --polljob --jobid <jobId> [--outfile string]
